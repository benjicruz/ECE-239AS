...
class OU_Noise:
    ...
    def _sample(self):
        ...
        # ========== YOUR CODE HERE ==========
        self.state += self.theta * (self.mu - self.state) + self.sigma * np.random.normal(size=self.action_space)
        return self.state
    ...
    def noise(self,action:np.ndarray[float]):
        ...
        # ====================================
        state = self._sample()
        noised_action = np.clip(action + state, self.action_range[0], self.action_range[1])
    ...
    def train(self, episodes:int, val_freq:int, val_episodes:int, test_episodes:int, save_every:int,
        ...
        for i in range(episodes):
            ...
            while not done:
                ...
                # ====================================
                action = self.actor(torch.tensor(state).float().to(self.device).unsqueeze(0)).detach().numpy()[0]
                action = self.OU_noise.noise(action)
