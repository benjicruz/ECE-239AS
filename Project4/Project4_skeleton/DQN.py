...
class DQN:
    ...
    def _optimize_model(self):
        ...
        with torch.no_grad():
            target_q = reward + self.gamma * (1 - done.float()) * self.model(next_state).max(1)[0]
    ...
    def _sample_action(self, state:np.ndarray
        ...
        if random.random() < epsilon:
            ...
            with torch.no_grad():
                ...
                index = self.model(state).argmax().item()
...
class HardUpdateDQN(DQN):
    ...
    def _optimize_model(self):
        ...
        with torch.no_grad():
            target_q = reward + self.gamma * (1 - done.float()) * self.target_model(next_state).max(1)[0]
...
class SoftUpdateDQN(HardUpdateDQN):
    ...
    def _update_model(self):
        ...
        if self.wandb:
            wandb.log({'tau': self.tau, 'i_update': self.i_update})
        self.i_update += 1
        if self.wandb:
            wandb.log({'i_update': self.i_update})
