...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_label = torch.ones(logits_real.size()).type(dtype)
    false_label = torch.zeros(logits_fake.size()).type(dtype)

    bce_real = bce_loss(logits_real, true_label)
    bce_fake = bce_loss(logits_fake, false_label)
...
def generator_loss(logits_fake):
    ...
    false_label = torch.ones(logits_fake.size()).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ls_real = (scores_real - 1)**2
    ls_fake = scores_fake ** 2

    loss = 0.5 * torch.mean(ls_real) + 0.5 * torch.mean(ls_fake)
...
def ls_generator_loss(scores_fake):
    ...
    ls_fake = (scores_fake - 1)**2
    loss = 0.5 * torch.mean(ls_fake)
