...
class CVAE(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        encoder_input = torch.cat([x, labels], dim=1)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    reconstruction_loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum') / x_hat.shape[0]
    D_KL = torch.sum((1 + logvar) - (mu**2) - (torch.exp(0.5 * logvar))**2) / logvar.shape[0]
    loss = 1 * reconstruction_loss + -0.5 * D_KL 
    ################################################################################################
    #                            END OF YOUR CODE                                                  #
    ################################################################################################
    return loss
