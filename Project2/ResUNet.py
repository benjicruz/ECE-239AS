...
class ConditionalUnet(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        # ==================================================== #
        init_conv = self.init_conv.forward(x)
        down1 = self.downblock1.forward(init_conv)
        fusion1 = self.fusion1.forward(down1, t=temb2, c=cemb2)
        down2 = self.downblock2.forward(fusion1)
        fusion2 = self.fusion2.forward(down2, t=temb1, c=cemb1)
        to_vec = self.to_vec(fusion2)
        up0 = self.upblock0(to_vec)
        up1 = self.upblock1.forward(up0, fusion2)
        fusion3 = self.fusion3.forward(up1, t=temb2, c=cemb2)
        up2 = self.upblock2.forward(fusion3, fusion1)
        fusion4 = self.fusion4.forward(up2, t=temb2, c=cemb2)
        out = self.outblock(torch.cat((up2, init_conv), dim = 1))
